...
function ui(){
 ...
 if(idx!==active){active=idx;$('name').textContent=o.name;$('label-name').textContent=o.name;$('object-info').setAttribute('aria-label',`About ${o.name}`);$('kind').textContent=o.kind;$('number').textContent=o.range?'∴':o.id==='sun'?'☉':String(objects.filter(p=>p.diameter).indexOf(o)).padStart(2,'0');$('size-label').textContent=o.hypothetical?'ILLUSTRATIVE DIAMETER':specimen?'APPROXIMATE LENGTH':'EQUATORIAL DIAMETER';$('size').textContent=o.hypothetical?'≈29,339 km · 2.3 Earths':specimen?`${specimen.length} km`:`${o.diameter.toLocaleString()} km`;$('distance-label').textContent=o.hypothetical?'ILLUSTRATIVE LOCATION':'MEAN DISTANCE FROM SOL';$('sol').textContent=o.id==='sun'?'0 km':o.hypothetical?'600 AU (possible)':`${o.au.toLocaleString(undefined,{maximumFractionDigits:3})} AU`;$('fact').textContent=facts[idx];
